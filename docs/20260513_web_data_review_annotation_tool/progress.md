...
## 2026-06-04
- Read `/home/dex/app/my_docs/20260520_umi_data_quality` and extracted the DTW MVP requirements: pairwise relative pose trajectory comparison, rot6d geodesic rotation error, configurable position/rotation scales, warp distortion, batch matrix, medoid, and outlier candidates.
- Added Phase 08 design doc for DTW trajectory quality evaluation. The proposed architecture introduces `univis.quality` abstractions and keeps DTW independent from concrete input formats by consuming `PolicyEpisode`.
- Updated the documentation index and task plan so DTW is tracked as a planned UniVis stage rather than a standalone note.
- Updated Phase 08 reference design: the frontend should let users set one current-directory episode as the expert/reference trajectory and keep using it until the user explicitly changes reference or switches dataset source.
- Reworked Phase 08 around the clarified product flow: DTW is an interactive viewer overlay in the Quality area below Annotation, not a report-first workflow. The design now covers the collapsible DTW block, enable toggle, current/medoid reference actions, 3D current-vs-reference alignment lines, draggable metrics popup, and selected-episode stats popup without report persistence.
- Implemented Phase 08 first pass. Added `univis.quality` with DTW models, dual-arm pose extraction, rot6d geodesic rotation error, O(MN) DTW, visual link decimation, medoid reference selection, and selected-episode stats aggregation.
- Added `/api/quality` routes for backend listing, current/reference compare, medoid reference selection, and selected stats. Registered `DTWTrajectoryQualityBackend` in the component registry.
- Added frontend Quality/DTW controls under Annotation, including collapsible DTW block, enable checkbox, current/medoid reference actions, selected stats popup, draggable metrics popup, and Plotly current/reference 3D alignment overlay.
- Verified Phase 08 with targeted tests and full suite: `50 passed`; also ran `python3 -m compileall src tests` and `node --check` for touched frontend files.
- Fixed DTW state reset when switching episodes. Root cause: episode switching temporarily unmounted the viewer/inspector branch, destroying `QualityPanel` local state. Lifted DTW enabled/reference/expanded state into `App`, and made `QualityPanel` clear only transient comparison state on real source changes. Re-verified with `50 passed`, `compileall`, and `node --check`.
- Added backend-managed structured DTW config support. DTW defaults now live in `src/univis/quality/config/dtw/default.yaml` and are loaded through `QualityConfig`, similar to the PIKA raw format config pattern. Removed frontend request-level DTW config controls so GUI and future CLI share the same backend file-managed behavior. Verified with `51 passed`, `compileall`, `node --check`, wheel resource inspection for the YAML file, and file line-count checks.
