...
class PikaRawEpisodeAdapter(RawEpisodeAdapter):
    ...
    def __init__(
        self,
        options: PikaSyncOptions | None = None,
        ...
        Inputs:
            ...
            config: Optional structured format config loaded from YAML.
        Output:
            Adapter with a small in-process synchronization cache.
        """

        self.config = (config or PikaRawFormatConfig.load()).with_sync_options(options)
        self.synchronizer = PikaEpisodeSynchronizer(self.config.sync)
    ...
    def _episode_dirs(self, source: EpisodeSource | None) -> list[Path]:
        ...
        return collect_pika_episode_dirs(source.root_path, self.config)
    ...
    def _sync(self, path: Path) -> PikaSyncResult:
        ...
        if episode_dir not in self._cache:
            self._cache[episode_dir] = self.synchronizer.synchronize(
                scan_pika_episode(episode_dir, self.config)
            )
    ...
    def _cameras(self, result: PikaSyncResult) -> list[CameraStream]:
        ...
        for camera in (self.config.layout.left_camera, self.config.layout.right_camera):
            paths = result.image_paths.get(camera.key, [])
            width, height = image_size(paths[0]) if paths else (640, 360)
            cameras.append(CameraStream(key=camera.key, label=camera.label, width=width, height=height))
        return cameras
