...
from univis.formats.pika_raw.manifest import PikaEpisodeManifest
...
class PikaEpisodeSynchronizer:
    ...
    def synchronize(self, manifest: PikaEpisodeManifest) -> PikaSyncResult:
        ...
        left_cam_ts, left_cam_paths = _list_timestamped_files(
            manifest.left_camera_dir, manifest.left_camera_suffixes
        )
        right_cam_ts, right_cam_paths = _list_timestamped_files(
            manifest.right_camera_dir, manifest.right_camera_suffixes
        )
        left_pose_ts, left_pose = _load_pose_series(manifest.left_pose_dir, manifest.pose_fields)
        right_pose_ts, right_pose = _load_pose_series(manifest.right_pose_dir, manifest.pose_fields)
        ...
        start, end = _trim_static_boundary_frames(left_pose_interp, right_pose_interp, opt)
        ...
        left_pose_out = _maybe_rebase_pose_series(left_pose_interp, opt)
        right_pose_out = _maybe_rebase_pose_series(right_pose_interp, opt)
        ...
        return PikaSyncResult(
            ...
            image_paths={manifest.left_camera_key: left_paths, manifest.right_camera_key: right_paths},
        )
...
def _load_pose_series(dir_path: Path, fields: tuple[str, ...]) -> tuple[np.ndarray, np.ndarray]:
    ts, paths = _list_timestamped_files(dir_path, [".json"])
    rows = []
    for path in paths:
        payload = _unwrap_payload(read_json(path))
        rows.append([float(payload[key]) for key in fields])
...
def _maybe_rebase_pose_series(pose_xyzrpy: np.ndarray, options: PikaSyncOptions) -> np.ndarray:
    """Convert pose rows to policy pose rows with optional first-frame rebase."""

    if options.rebase_to_first_frame:
        return _rebase_pose_series_to_first_frame(pose_xyzrpy)
    return matrix_to_pose_xyz6d(pose_xyzrpy_to_matrix(pose_xyzrpy))


def _trim_static_boundary_frames(left: np.ndarray, right: np.ndarray, options: PikaSyncOptions) -> tuple[int, int]:
    if not options.trim_static_boundaries:
        return 0, left.shape[0]
    trans, rot = options.static_translation_m, math.radians(options.static_rotation_deg)
